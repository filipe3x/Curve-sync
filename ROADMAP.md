...
- **Referência:** `docs/embers-reference/curve.py`
- **Destino:** `server/src/services/emailParser.js`

### 1.2 Leitor IMAP directo
Implementar ligação IMAP directa (substituindo offlineimap) para ler emails da pasta configurada. Usar a flag `UNSEEN` para saber quais emails já foram processados. Marcar como `Seen` apenas após processamento com sucesso.

- **Destino:** `server/src/services/imapReader.js`
- **Endpoints:** `POST /api/curve/sync`, `POST /api/curve/test-connection`

### 1.3 Orquestrador de sincronização
Serviço que coordena: ligar IMAP → buscar emails não lidos → parsing → calcular digest → verificar duplicados → inserir despesa → criar log → marcar email como lido. Deve tratar erros por email individualmente (um email com erro não bloqueia os restantes).

- **Destino:** `server/src/services/syncOrchestrator.js`

### 1.4 Sincronização automática (node-cron)
...
### 1.5 Autenticação / Scoping por utilizador
...
### 2.1 Relatórios mensais com ciclo dia 22
...
### 2.3 Validação de campos extraídos
...
### 2.5 Filtros avançados na listagem de despesas
...
### 2.6 Encriptação de credenciais IMAP
